...
// 할일 목록 배열을 통해서 전체적인 할일 목록 ui를 생성
// 할일 목록 todoList와 그려줄 영역 parent를 넘겨받아
...
function drawTodoList(parent, list){
    ...
    for(let unit of list){
        ...
        const toDoLi = document.createElement('li');
        toDoLi.isDone = false;
        toDoLi.innerHTML = unit.title;// 할일에 대한 텍스트;
        parent.appendChild(toDoLi);

        // 할일 목록을 지워줄 버튼을 만들어서 li태그 자식노드로 추가
        const removeToDoli = document.createElement('div');
        removeToDoli.className = 'todo-remove-btn';
        removeToDoli.innerHTML = '<i class="fa-solid fa-eraser"></i>';
        toDoLi.appendChild(removeToDoli);
